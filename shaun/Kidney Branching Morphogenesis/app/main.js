/*@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
 * WEBGL 2.0    :   KIDNEY BRANCHING MORPHOGENESIS MODEL
 *
 ...
 */
    ...
    gui.dspPrmFldr.nclimits.add(env, 'xmin').name('[R]-min').onChange( function(){
            env.nc.plt.xlimits = [env.xmin,env.xmax] ;
            env.nc.run() ;
            refreshDisplay();
        } ) ;
    gui.dspPrmFldr.nclimits.add(env, 'xmax').name('[R]-max').onChange( function(){
            env.nc.plt.xlimits = [env.xmin,env.xmax] ;
            env.nc.run() ;
            refreshDisplay();
        } ) ;
    gui.dspPrmFldr.nclimits.add(env, 'ymin').name('[G]-min').onChange( function(){
            env.nc.plt.ylimits = [env.ymin,env.ymax] ;
            env.nc.run() ;
            refreshDisplay();
        } ) ;
    gui.dspPrmFldr.nclimits.add(env, 'ymax').name('[G]-max').onChange( function(){
            env.nc.plt.ylimits = [env.ymin,env.ymax] ;
            env.nc.run() ;
            refreshDisplay();
        } ) ;
 ...
 */
    ...
    nc.crv1.name = 'R-nullcline' ;
    nc.crv2 = nc.plt.addCurveFromPoints() ;
    nc.crv2.name = "G-nullcline" ;
    ...
    nc.run = function(){
        ...
        for(var v=nc.plt.xlimits[0]; v<nc.plt.xlimits[1] ; v+=0.01){
            var w = (-env.p_R + env.d_R*v)/((env.k*env.p_c - 2*env.k*env.d_c)*(v**2)) ;
            nc.crv1.plot(v,w) ;
            w = (env.p_G0 + env.p_G*((env.w0)**2)/(((env.w0)**2) + env.k_W))/(env.d_G + env.k*env.d_c*(v**2)) ;
            nc.crv2.plot(v,w) ;
        }

        var v ,w  ;
        nc.crv3.plot(nc.v, nc.w) ;
        for( var t =0 ; t < 10 ; t+=env.dt){
            v =nc.v ; w =nc.w ;
            nc.dv = env.dt*(env.p_R - env.d_R*v + (env.k*env.p_c - 2*env.k*env.d_c)*(v*v)*w) ;
            nc.dw = env.dt*(env.p_G0 + env.p_G*((env.w0)*(env.w0))/(((env.w0)*(env.w0)) + env.k_W) - env.d_G*w - env.k*env.d_c*(v*v)*w) ;
 ...
 */
    env.compUniforms = function(_inUv ){
        ...
        wrapS: 'repeat', wrapT: 'repeat'} ;
        this.Dr   = { type : 'f',     value   : env.Dr    } ;
        this.Dg   = { type : 'f',     value   : env.Dg    } ;
        this.p_R  = { type : 'f',     value   : env.p_R   } ;
        this.d_R  = { type : 'f',     value   : env.d_R   } ;
        this.k    = { type : 'f',     value   : env.k     } ;
        this.dx   = { type : 'f',     value   : env.dx    } ;
        this.d_c  = { type : 'f',     value   : env.d_c   } ;
        this.p_G0 = { type : 'f',     value   : env.p_G0  } ;
        this.dt   = { type : 'f',     value   : env.dt    } ;
        this.p_c  = { type : 'f',     value   : env.p_c   } ;
        this.p_G  = { type : 'f',     value   : env.p_G   } ;
        this.w0    = { type : 'f',     value   : env.w0     } ;
        this.k_W  = { type : 'f',     value   : env.k_W   } ;
        this.d_G  = { type : 'f',     value   : env.d_G   } ;
 ...
 */
    ...
    env.disp.addMessage(  "[R]-variable",
