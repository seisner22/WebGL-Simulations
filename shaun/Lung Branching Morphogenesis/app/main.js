/*@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
 * WEBGL 2.0    :   2-D 2v Lung Branching Morphogenesis Model
 *
 * PROGRAMMER   :   ABOUZAR KABOUDIAN, Shaun Eisner
 * DATE         :   Mon 1 Jul 2019 16:19:48 EDT
 ...
 */
    ...
    gui.dspPrmFldr.nclimits.add(env, 'xmin').name('L-min').onChange( function(){
            env.nc.plt.xlimits = [env.xmin,env.xmax] ;
            env.nc.run() ;
            refreshDisplay();
        } ) ;
    gui.dspPrmFldr.nclimits.add(env, 'xmax').name('L-max').onChange( function(){
            env.nc.plt.xlimits = [env.xmin,env.xmax] ;
            env.nc.run() ;
            refreshDisplay();
        } ) ;
    gui.dspPrmFldr.nclimits.add(env, 'ymin').name('R-min').onChange( function(){
            env.nc.plt.ylimits = [env.ymin,env.ymax] ;
            env.nc.run() ;
            refreshDisplay();
        } ) ;
    gui.dspPrmFldr.nclimits.add(env, 'ymax').name('R-max').onChange( function(){
            env.nc.plt.ylimits = [env.ymin,env.ymax] ;
            env.nc.run() ;
            refreshDisplay();
        } ) ;
 ...
 */
    ...
    gui.click.add( env,  'setV').name('Set R?').onChange(function(){
        env.click.uniforms.setV.value = env.setV ;
    });
    gui.click.add(env,  'v').name('R value').onChange(function(){
        env.click.uniforms.v.value = env.v ;
        env.nc.run() ;
    } ) ;
    gui.click.add( env,  'setW').name('Set L?').onChange(function(){
        env.click.uniforms.setW.value = env.setW ;
    });
    gui.click.add(env,  'w').name('L value').onChange(function(){
 ...
 */
    ...
    nc.crv1.name = 'L-nullcline' ;
    nc.crv2 = nc.plt.addCurveFromPoints() ;
    nc.crv2.name = "R-nullcline" ;
    ...
    nc.run = function(){
        ...
        for( var t =0 ; t < 30 ; t+=env.dt){
            v =nc.v ; w =nc.w ;
            nc.dv = env.dt*(env.gamma*(env.b - v + v*v*w)) ;
            nc.dw = env.dt*(env.gamma*(env.a - w - v*v*w)) ;
            nc.v += nc.dv ;
            nc.w += nc.dw ;
            nc.crv3.plot(nc.v, nc.w) ;
        }
 ...
 */
    env.compUniforms = function(_inUv ){
        ...
        this.gamma      = { type : 'f',     value   : env.delta     } ;
