 ...
 */
    ...
    nc.run = function(){
        ...
        for(var v=nc.plt.xlimits[0]; v<nc.plt.xlimits[1] ; v+=0.01){
            var w = 1./(env.t_res*(v**2. + (1./env.t_res))) ;
            nc.crv1.plot(v,w) ;
            w = 1./(env.t2*v) + (v - env.b0)/(env.t_res*(v**2.)) ;
            nc.crv2.plot(v,w) ;
        }
        ...
        for( var t =0 ; t < 30 ; t+=env.dt){
            v =nc.v ; w =nc.w ;
            nc.dv = env.dt*(w*(v**2) - v/(env.t2) - (v - env.b0)/env.t_res) ;
            nc.dw = env.dt*(-w*(v**2) + (1. - w)/(env.t_res)) ;
 ...
 */
    env.compUniforms = function(_inUv ){
        ...
        this.t_res= { type : 'f',     value   : env.t_res } ;
        this.t2   = { type : 'f',     value   : env.t2    } ;
        this.b0   = { type : 'f',     value   : env.b0    } ;
