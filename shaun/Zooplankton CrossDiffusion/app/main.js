 ...
 */
    ...
    gui.dspPrmFldr.nclimits.add(env, 'xmin').name('x-min').onChange( function(){
            env.nc.plt.xlimits = [env.xmin,env.xmax] ;
            env.nc.run() ;
            refreshDisplay();
        } ) ;
    gui.dspPrmFldr.nclimits.add(env, 'xmax').name('x-max').onChange( function(){
            env.nc.plt.xlimits = [env.xmin,env.xmax] ;
            env.nc.run() ;
            refreshDisplay();
        } ) ;
    gui.dspPrmFldr.nclimits.add(env, 'ymin').name('y-min').onChange( function(){
            env.nc.plt.ylimits = [env.ymin,env.ymax] ;
            env.nc.run() ;
            refreshDisplay();
        } ) ;
    gui.dspPrmFldr.nclimits.add(env, 'ymax').name('y-max').onChange( function(){
            env.nc.plt.ylimits = [env.ymin,env.ymax] ;
            env.nc.run() ;
            refreshDisplay();
        } ) ;
 ...
 */
    ...
    gui.click.add( env,  'setV').name('Set x?').onChange(function(){
        env.click.uniforms.setV.value = env.setV ;
    });
    gui.click.add(env,  'v').name('x value').onChange(function(){
        env.click.uniforms.v.value = env.v ;
        env.nc.run() ;
    } ) ;
    gui.click.add( env,  'setW').name('Set x?').onChange(function(){
        env.click.uniforms.setW.value = env.setW ;
    });
    gui.click.add(env,  'w').name('y value').onChange(function(){
 ...
 */
    ...
    nc.crv1.name = 'P-nullcline' ;
    nc.crv2 = nc.plt.addCurveFromPoints() ;
    nc.crv2.name = "Z-nullcline" ;
    ...
    nc.run = function(){
        ...
        nc.w = env.w ;
        nc.plt.reset() ;
        for(var x=nc.plt.xlimits[0]; x<nc.plt.xlimits[1] ; x+=0.01){
            var y = (env.r*x*(1 - (x/env.k))*(env.h + x)/(env.beta*x));
            nc.crv1.plot(x,y) ;
            y = x ;
            nc.crv2.plot((-env.m*env.h)/(env.delta),y) ;
        }
        ...
        for( var t =0 ; t < 20 ; t+=env.dt){
            v =nc.v ; w =nc.w ;
            nc.dv = env.dt*((env.alpha*w*v)/(env.h + w) - env.m*v - (env.delta*w*v)/(env.h + w)) ;
            nc.dw = env.dt*(env.r*w*(1 - (w/env.k)) - (env.beta*w*v)/(env.h + w)) ;
            nc.v += nc.dv ;
            nc.w += nc.dw ;
            nc.crv3.plot(nc.w, nc.v) ;
        }
 ...
 */
    env.compUniforms = function(_inUv ){
        ...
        wrapS: 'repeat', wrapT: 'repeat'} ;
        this.D11        = { type : 'f',     value   : env.D11         } ;
        this.D12        = { type : 'f',     value   : env.D12         } ;
        this.D21        = { type : 'f',     value   : env.D21         } ;
        this.D22        = { type : 'f',     value   : env.D22         } ;
        this.r          = { type : 'f',     value   : env.r           } ;
        this.k          = { type : 'f',     value   : env.k           } ;
        this.beta       = { type : 'f',     value   : env.beta        } ;
        this.alpha      = { type : 'f',     value   : env.alpha       } ;
        this.m          = { type : 'f',     value   : env.m           } ;
        this.delta      = { type : 'f',     value   : env.delta       } ;
        this.h          = { type : 'f',     value   : env.h           } ;
 ...
 */
    ...
    env.disp.addMessage(  "Z-variable",
