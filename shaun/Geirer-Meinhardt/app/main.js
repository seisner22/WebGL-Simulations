/*@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
 * WEBGL 2.0    :   GEIRER-MEINHARDT  MODEL
 *
 ...
 */
    ...
    gui.dspPrmFldr.nclimits.add(env, 'xmin').name('a-min').onChange( function(){
            env.nc.plt.xlimits = [env.xmin,env.xmax] ;
            env.nc.run() ;
            refreshDisplay();
        } ) ;
    gui.dspPrmFldr.nclimits.add(env, 'xmax').name('a-max').onChange( function(){
            env.nc.plt.xlimits = [env.xmin,env.xmax] ;
            env.nc.run() ;
            refreshDisplay();
        } ) ;
    gui.dspPrmFldr.nclimits.add(env, 'ymin').name('h-min').onChange( function(){
            env.nc.plt.ylimits = [env.ymin,env.ymax] ;
            env.nc.run() ;
            refreshDisplay();
        } ) ;
    gui.dspPrmFldr.nclimits.add(env, 'ymax').name('h-max').onChange( function(){
            env.nc.plt.ylimits = [env.ymin,env.ymax] ;
            env.nc.run() ;
            refreshDisplay();
        } ) ;
 ...
 */
    ...
    gui.click.add( env,  'setV').name('Set h?').onChange(function(){
        env.click.uniforms.setV.value = env.setV ;
    });
    gui.click.add(env,  'v').name('h value').onChange(function(){
        env.click.uniforms.v.value = env.v ;
        env.nc.run() ;
    } ) ;
    gui.click.add( env,  'setW').name('Set a?').onChange(function(){
        env.click.uniforms.setW.value = env.setW ;
    });
    gui.click.add(env,  'w').name('a value').onChange(function(){
 ...
 */
    ...
    nc.crv1.name = 'a-nullcline' ;
    nc.crv2 = nc.plt.addCurveFromPoints() ;
    nc.crv2.name = "h-nullcline" ;
    ...
    nc.run = function(){
        ...
        nc.w = env.w ;
        nc.plt.reset() ;
        for(var a=nc.plt.xlimits[0]; a<nc.plt.xlimits[1] ; a+=0.01){
            var h = (env.c*env.q*(a**2))/((env.mu*a - env.q0*env.q)*(1.0 + env.kappa*a));
            nc.crv1.plot(a,h) ;
            h = env.cp*env.qp*(a**2)/env.v0 ;
            nc.crv2.plot(a,h) ;
        }
        ...
        for( var t =0 ; t < 20 ; t+=env.dt){
            v =nc.v ; w =nc.w ;
            nc.dv = env.dt*(env.cp*env.qp*(w**2.) - v*env.v0) ;
            nc.dw = env.dt*(env.q0*env.q + (env.c*env.q*(w**2.))/(v*(1. + env.kappa*w)) - env.mu*w) ;
            nc.v += nc.dv ;
            nc.w += nc.dw ;
            nc.crv3.plot(nc.v, nc.w) ;
        }
 ...
 */
    env.compUniforms = function(_inUv ){
        ...
        wrapS: 'mirrored_repeat', wrapT: 'mirrored_repeat'} ;
        this.Da         = { type : 'f',     value   : env.Da          } ;
        this.Dh         = { type : 'f',     value   : env.Dh          } ;
        this.q         = { type : 'f',     value   : env.q          } ;
        this.c         = { type : 'f',     value   : env.c          } ;
        this.q0         = { type : 'f',     value   : env.q0          } ;
        this.v0         = { type : 'f',     value   : env.v0          } ;
        this.kappa         = { type : 'f',     value   : env.kappa          } ;
        this.qp         = { type : 'f',     value   : env.qp          } ;
        this.mu          = { type : 'f',     value   : env.b           } ;
        this.cp          = { type : 'f',     value   : env.cp           } ;
 ...
 */
    ...
    env.disp.addMessage(  "a-variable",
