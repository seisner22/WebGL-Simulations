 ...
 */
    ...
    gui.click.add( env,  'setV').name('Set Y?').onChange(function(){
        env.click.uniforms.setV.value = env.setV ;
    });
    gui.click.add(env,  'v').name('Y value').onChange(function(){
    ...
    gui.click.add(env,  'w').name('X value').onChange(function(){
 ...
 */
    ...
    nc.crv1.name = 'X-nullcline' ;
    nc.crv2 = nc.plt.addCurveFromPoints() ;
    nc.crv2.name = "Y-nullcline" ;
    ...
    nc.run = function(){
        ...
        for( var t =0 ; t < 20 ; t+=env.dt){
            v =nc.v ; w =nc.w ;
            nc.dv = env.dt*(env.c*w + env.d*v) ;
            nc.dw = env.dt*(env.a*w + env.b*v) ;
            nc.v += nc.dv ;
            nc.w += nc.dw ;
            nc.crv3.plot(nc.v, nc.w) ;
        }
 ...
 */
    env.compUniforms = function(_inUv ){
        ...
        wrapS: 'repeat', wrapT: 'repeat'} ;
        this.mux         = { type : 'f',     value   : env.mux          } ;
        this.muy         = { type : 'f',     value   : env.muy          } ;
 ...
 */
    ...
    env.disp.addMessage(  "X-variable",
